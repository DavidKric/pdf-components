...
export const DocumentWrapper: React.FunctionComponent<Props> = ({
  ...
  React.useEffect(() => {
    ...
    setLastScale(scale);
  }, [scale, lastScale, updateScrollPosition]);
  ...
  }, [setNumPagesLoaded, setNumPages, setPageDimensions, setErrorMessage, setIsLoading, pdfDocProxy, setPdfDocProxy]);
  ...
  const onItemClicked = (param: Destination): void => {
    ...
    pdfDocProxy.getDestination(param.dest).then((destArray: any) => {
      if (!destArray) {
        return;
      }

      const [ref, , , bottomPoints] = destArray;
      pdfDocProxy.getPageIndex(new Ref(ref)).then((refInfo: any) => {
